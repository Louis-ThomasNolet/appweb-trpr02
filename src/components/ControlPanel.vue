...
watch(
  () => currentEnemy.value?.ship.vitality,
  () => {
...
function repairShip() {
  if (
    player.value?.selectedShip.vitality < 100 &&
    player.value.selectedShip.vitality > 0 &&
    currentEnemy.value.ship.vitality <= 0
  ) {
    while (
      player.value.credits > 0 &&
      player.value.credits >= 2 &&
      player.value.selectedShip.vitality < 100
    ) {
    ...
    <div class="card border-0">
      ...
      <div class="card-body text-white bg-dark">
        <div class="row">
          ...
          <div class="col-4">
            <button class="btn btn-primary" @click="endMission()">
              Terminer la mission
            </button>
          </div>
          <div class="col-4">
            <button class="btn btn-primary" @click="repairShip()">
